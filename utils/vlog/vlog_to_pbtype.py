...
The following are allowed on nets within modules (TODO: use proper Verilog timing):
    ...
    - `(* DELAY_CONST_{input}="30e-12" *)` : specify a constant max delay from an
         input (applied to the output)
    ...
    def make_pb_content(mod, xml_parent, mod_pname, is_submode=False):
        ...
        if (not is_blackbox) or is_submode:
            ...
            for cname, i_of in mod.cells:
                ...
                # In order to avoid overspecifying interconnect, there
                # are two directions we currently consider. All
                # interconnect going INTO a cell, and interconnect
                # going out of a cell into a top level output - or all
                # outputs if "mode" is used.
                inp_cons = mod.cell_conns(cname, "input")
                for pin, net in inp_cons:
                    drvs = mod.net_drivers(net)
                    if len(drvs) == 0:
                        logging.critical("Pin %s.%s has no driver, interconnect will be missing",
                                         pb_name, pin)
                    elif len(drvs) > 1:
                        logging.critical("Pin %s.%s has multiple drivers," +
                ...
                for pin, net in out_cons:
                    ...
                    for sink_cell, sink_pin in sinks:
                        if sink_cell == mod.name:
                             #Only consider outputs from cell to top
                             #level IO. Inputs to other cells will be
                             #dealt with in those cells.
        ...
        # Process timing
        for name, _, _ in mod.ports:
            port = "{}.{}".format(mod_pname, name)
            # Clocked timing
            t_setup = mod.net_attr(name, "SETUP")
            t_hold = mod.net_attr(name, "HOLD")
            t_ctoq = mod.net_attr(name, "CLK_TO_Q")
            process_clocked_tmg(t_setup, port, "T_setup", xml_parent)
            process_clocked_tmg(t_hold, port, "T_hold", xml_parent)
            process_clocked_tmg(t_ctoq, port, "T_clock_to_Q", xml_parent)
    ...
    def make_pb_type(mod):
        ...
        if has_modes:
            for mode in modes:
                ...
                mode_yj = YosysJSON(yosys.run.vlog_to_json(args.infiles, flatten=False, aig=False,
                                                           mode=smode, module_with_mode=mod.name))
