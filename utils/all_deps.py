...
def gen_rules_Ntemplate(in_dir, base=None):
  fmt = """#template expansion rule
{output}: {template} {makefile} {cmd}
\t$(QUIET){cmd} {nvalue} {template} {output}
N_OUTPUTS += {output}
"""
  ...
  outputs = []
  if base is None:
    tfiles = [xx for xx in os.listdir(in_dir) if xx.startswith(TEMPLATE_PREFIX)]
  else:
    tfiles = [base]
  for nv in nvalues:
    for templatefile in tfiles:
      out_base = templatefile.replace(TEMPLATE_PREFIX,'').replace('N', nv)
      template = os.path.join(in_dir, templatefile)
      output = os.path.join(out_dir, out_base)
      logging.debug('generating rule %s: %s', output, template)
      res += fmt.format(output=output, template=template, cmd=CMD, makefile=makefile, nvalue=nv)
...
.PHONY: type{type} type{type}-clean
"""
  res = '.PHONY: clean\n'
...
def gen_deps(ff):
  ...
  N_outputs = []
  for dirname in trad:
    logging.debug('expanding templates %s', os.path.basename(dirname))
    rules, outputs = gen_rules_Ntemplate(dirname)
    ff.write(rules)
    N_outputs += outputs
  ...
  Ngen_outputs = []
  for dirname in gend:
    for suf in [SIMV_SUFFIX, PB_SUFFIX, MODEL_SUFFIX]:
      opts = mux_gen_check_args(os.path.join(dirname, 'Makefile.mux'))
      name = opts[MUX_OUTFILE_KEY] + '.' + suf
